...
import { HiUser, HiOutlineHome, HiOutlineSpeakerphone, HiPhone, HiOutlineInformationCircle, HiOutlineDocumentText, } from "react-icons/hi";
import {
   Bars3Icon,
   ComputerDesktopIcon,
...
const NavBar = () => {
   ...
   const navLinks = [
      ...
      { label: "Property Listing", path: "/property-listing", icon: HiOutlineDocumentText },
   ];
      ...
      <div className="fixed top-0 w-full bg-[#232323] lg:bg-black flex items-center p-4 z-50 h-[63px] lg:h-[62px] justify-between pt-6">
         ...
         <ul className="hidden sm:flex items-center font-medium lg:text-sm space-x-6">
            {navLinks.map((link, index) => (
               ...
               >
                  <li
                     className={`py-1 hover:bg-teal-500 hover:text-white hover:rounded-md text-md px-3 ${activeNavbarMenu === link.label
            ...
            <div>
               {authState.status ? (
                  ...
                  <button
                     ...
                     className={`py-1 hover:bg-teal-500 hover:text-white px-3 hover:rounded-md text-md ${activeNavbarMenu === "login"
                        ? "bg-teal-500 text-white rounded-md"
                        : ""
                        }`}
                  >
         ...
         {/* Mobile Navigation */}
         <div
            className={` w-[90%] sm:hidden fixed inset-0 bg-[#232323] text-white z-[1000] transition-transform ${showMenu ? "translate-x-0 " : "-translate-x-full"}`}
         >

            <div className="">
               <div
                  className={`max-sm:pt-1 w-[100%] h-[100%] absolute sm:h-[100%] sm:w-[100%] bg-[#232323] sm:bg-black text-white sm:p-2 flex flex-col sm:relative sm:top-0 sm:left-0 lg:p-5 z-50 pl-0  transition-all duration-200 ${sidebar ? "translate-x-[0%] w-[100%] visible" : "translate-x-[100%] invisible w-0"}`}
               >
                  <div className={`flex flex-col h-screen w-[100%] sm:w-auto sm:justify-normal `}>
                     <div className="w-[100%] sm:w-auto">

                        <div className={`flex text-white-700 ${linkStyle_1}`} onClick={() => {
                           setSidebar(false);
                        ...
                        >
                           <NavLink
                              ...
                              className={({ isActive }) =>
                                 ` max-sm:w-[100%] sm:rounded-md px-[14px] py-[10px] flex items-center gap-x-4 h-[45px] ${isActive ? "bg-[#C8A117]" : ""
                                 } hover:bg-[#C8A117] transition-colors duration-200`
                              }
                              onClick={(e) => {
                                 e.stopPropagation();
                              ...
                              {toggelSetting && (
                                 <ul className="flex-col max-sm:ml-auto gap-y-[2px] lg:mt-4 lg:ml-8 relative max-sm:top-full max-sm:right-0 max-sm:w-[69%] max-sm:bg-black max-sm:z-50 max-sm:justify-around max-sm:p-2 ">
                                    <NavLink
                                       to="/landlord-dashboard/settings/profile"
                                       className={({ isActive }) =>
                                          `${linkStyle_2} ${isActive ? "bg-[#C8A117]" : ""} max-sm:flex-1 max-sm:justify-center w-full hover:bg-[#C8A117] transition-colors duration-200`
                                       }
                                       onClick={(e) => {
                                          e.stopPropagation();
                                          setToggelSetting(false);
                                    ...
                                    >
                                       <li className="flex max-sm:justify-center">
                                          ...
                                          <span className="text-base sm:block lg:text-xl">Profile</span>
                                       </li>
                                    </NavLink>
                                    ...
                                    >
                                       <li className="flex max-sm:justify-center">
                                          ...
                                          <span className="text-base sm:block lg:text-xl">Account Security</span>
                                       </li>
                                    </NavLink>
                                 </ul>
                              )}
            ...
            <div className={`bg-[#272727] flex items-center justify-between py-4 relative`} onClick={() => {
               setSidebar(true);
            }}>

               <div className={`absolute z-[999] transition-all top-0 ${showMenu ? "ml-[100%]" : ""} rounded-2xl border border-white`} onClick={() => {
                  setShowMenu(false);
                  if (sidebar) {
                     setSidebar(false);
                  }
               ...
               {navLinks.map((link, index) => (
                  ...
                  >
                     <div className="flex items-center gap-x-3">
                        <p><link.icon size={25} /></p>
                        <p>{link.label}</p>
                     </div>
                  </NavLink>
               ))}

               <div className="flex items-center gap-x-3 text-red-500" onClick={handleLogout}>
                  <FontAwesomeIcon
                     icon={faArrowRightFromBracket}
                     className="text-xl "
                  />
