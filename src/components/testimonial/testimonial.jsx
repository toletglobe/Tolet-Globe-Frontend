      ...
      <div className="flex justify-center gap-x-40 mt-10">
        ...
        <Slider {...settings} className="w-[400px] h-[500px] px-3 md:px-8 lg:px-1">
          {images.map((d, index) => (
            <div
              key={index}
              className="bg-white p-4 rounded-lg shadow-lg w-0 h-[500px] md:w-[300px] text-left flex flex-col justify-between"
            >
              <img
                src={d.image}
                alt={d.name}
              ...
              <p className="text-black mb-2 h-[96px] overflow-hidden">
                {d.review}
              </p>
              {/* Changed margin-top to align logos slightly higher */}
              <div className="flex space-x-2 justify-end mt-2">
                {/* Reduced margin-top for the logos */}
                <img
                  src={linked_in}
                  alt="LinkedIn"
