...
const Card = ({ img, title, bg, path, description }) => {
  ...
  return (
    <div onClick={() => navigate(path)} className="group cursor-pointer">
      {/* Mobile & Tablet View - Slightly Reduced Image Size */}
      <div className="relative bg-white rounded-lg overflow-hidden shadow-lg hover:shadow-xl w-full transition-shadow duration-300 cursor-pointer lg:hidden">
        <div className="flex justify-center items-center p-4 sm:pl-[59.18px] sm:pr-[59.18px] sm:pt-[23px] sm:pb-[23px]">
          <img 
            src={img} 
            className="h-[50px] w-[50px] sm:h-[64px] sm:w-[64px] object-contain" 
            alt={title}
          />
        </div>
        <div className={`rounded-b-lg text-center transition-all duration-300 group-hover:bg-opacity-80 min-h-[3.5rem] flex items-center justify-center px-1 ${bg}`}>
          <h1 className="text-base sm:text-lg md:text-xl font-bold whitespace-normal leading-tight">
            {title}
          </h1>
        </div>
        <div className={`absolute inset-0 flex flex-col items-center justify-center p-4 ${bg} bg-opacity-90 opacity-0 group-hover:opacity-100 transition-opacity duration-300`}>
      ...
      <div className="hidden lg:block w-[277px] h-[280px] rounded-[10px] overflow-hidden relative flex flex-col items-start justify-center text-left transition-all duration-300 hover:scale-105">
        ...
        <div className={`w-full h-[50%] flex flex-col justify-center rounded-b-[10px] p-4 ${bg}`}>
