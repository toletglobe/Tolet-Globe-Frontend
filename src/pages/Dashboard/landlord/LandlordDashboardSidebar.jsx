...
export default function LandlordDashboardSidebar() {
  ...
  const [sideSymbol, setSideSymbol] = useState(true);
  function sideIcon() {
    // alert("hello sidebar");
    if (sideStatus) {
      document.getElementById("sidebar").classList.remove("left-[0px]");
      document.getElementById("sidebar").classList.add("left-[-700px]");
      document.getElementById("sidebar").style.transition = "all .5s 0s linear";
    } else {
      document.getElementById("sidebar").classList.add("left-[0px]");
      document.getElementById("sidebar").classList.remove("left-[-700px]");
      document.getElementById("sidebar").style.transition = "all .5s 0s linear";
    }
  }
  ...
  return (
    <>
      ...
      >
        {sideSymbol ? (
          <i className="fa-solid fa-angle-right text-3xl md:hidden text-[#C8A117] "></i>
        ) : (
          <i className="fa-solid fa-xmark text-3xl md:hidden text-[#C8A117] "></i>
        )}
      </span>
      {/* sidebar icon end  */}
      ...
      <div
        ...
        id="sidebar"
      >
        <div className="flex flex-col h-screen w-screen sm:w-auto justify-between sm:justify-normal">
          <div className="w-[100%] sm:w-auto">
            ...
            >
              ...
              <div id="settingMenus">
                {toggelSetting && (
                  <ul className=" flex-col max-sm:ml-5 gap-y-[2px] lg:mt-4 lg:ml-8 max-sm:absolute max-sm:top-full max-sm:right-0 max-sm:w-[69%] max-sm:bg-black max-sm:z-10 max-sm:justify-around max-sm:p-2">
                    <NavLink
                      to="settings/profile"
                      className={({ isActive }) =>
                        `${linkStyle_2} ${
                          isActive ? "bg-[#C8A117]" : ""
                        } max-sm:flex-1 max-sm:justify-center w-[100%] hover:bg-[#C8A117] transition-colors duration-200`
                      }
                      onClick={() => {
                        setToggelSetting(false);
                        setArrow();
